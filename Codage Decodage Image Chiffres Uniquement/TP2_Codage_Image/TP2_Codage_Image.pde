...
String getColorString(final color c) {
  if (red(c) == 255 && green(c) == 255 && blue(c) == 255) return "FF";
  return "00";
}
    ...
    if (c == previousC) {
      if (repetition >= 32767) {
        ...
        if (difference > 3) {
          ...
          output = output + coderDifference(difference-3) + differentChars.substring(0, differentChars.length()-6);
        }
        difference = 1;
        differentChars="";
      } else {
