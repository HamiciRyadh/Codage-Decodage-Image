...
PImage decoderImage(final String INPUT) {
  ...
  while (i < INPUT.length()) {
    repetition = determinerNombreRepetition(INPUT.substring(i,i+7));
    if (repetition > 32768) {
      repetition = repetition - 32768;
      value = valueHexa(INPUT.substring(i+7,i+9));
...
int determinerNombreRepetition(final String INPUT) {
  int value = Integer.parseInt(INPUT.substring(1, 5), 16);
  return value;
}
